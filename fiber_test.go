...
func TestCtx(t *testing.T) {
	...
	request := httptest.NewRequest("GET", "/hello?name=Nafi", nil)
	response, err := app.Test(request)
	assert.Nil(t, err)
	assert.Equal(t, 200, response.StatusCode)
	...
	bytes, err := io.ReadAll(response.Body)
	assert.Nil(t, err)
	assert.Equal(t, "Hello Nafi", string(bytes))
...
func TestHttpRequest(t *testing.T) {
	...
	request.AddCookie(&http.Cookie{Name: "lastname", Value: "Nafi"})
	response, err := app.Test(request)
	assert.Nil(t, err)
	assert.Equal(t, 200, response.StatusCode)
	...
	bytes, err := io.ReadAll(response.Body)
	assert.Nil(t, err)
	assert.Equal(t, "Hello Muhammad Nafi", string(bytes))
}
...
func TestRouteParameter(t *testing.T) {
	...
	request := httptest.NewRequest("GET", "/users/nafi/orders/10", nil)
	response, err := app.Test(request)
	assert.Nil(t, err)
	assert.Equal(t, 200, response.StatusCode)
	...
	bytes, err := io.ReadAll(response.Body)
	assert.Nil(t, err)
	assert.Equal(t, "Get Order 10 From User nafi", string(bytes))
}
...
func TestFormRequest(t *testing.T) {
	...
	body := strings.NewReader("name=nafi")
...
func TestRequestBody(t *testing.T) {
	...
	body := strings.NewReader(`{"username":"nafi","password":"rahasia"}`)
...
func TestBodyParserJSON(t *testing.T) {
	...
	body := strings.NewReader(`{"username":"nafi","password":"rahasia","name":"Muhammad Nafi"}`)
	request := httptest.NewRequest("POST", "/login", body)
	request.Header.Set("Content-Type", "application/json")
	response, err := app.Test(request)
	assert.Nil(t, err)
	...
	bytes, err := io.ReadAll(response.Body)
	assert.Nil(t, err)
	assert.Equal(t, "Register Success nafi", string(bytes))
}
...
func TestBodyParserXML(t *testing.T) {
	...
	body := strings.NewReader(
		`
		<RegisterRequest>
			<username>nafi</username>
			<password>rahasia</password>
			<name>Muhammad Nafi</name>
		</RegisterRequest>
		`,
	)
	request := httptest.NewRequest("POST", "/login", body)
...
func TestResponseJSON(t *testing.T) {
	...
	bytes, err := io.ReadAll(response.Body)
	assert.Nil(t, err)
	assert.Equal(t, `{"name":"Muhammad Nafi","username":"nafi"}`, string(bytes))
}
...
func TestRoutingGroup(t *testing.T) {
	...
	api.Get("/world", helloWorld) // /api/hello

	web := app.Group("/web")
	web.Get("/hello", helloWorld) // /web/hello
	web.Get("/world", helloWorld) // /web/hello
